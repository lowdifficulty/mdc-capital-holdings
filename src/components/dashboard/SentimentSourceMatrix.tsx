import type { SentimentLabel } from "@/lib/sentiment/types";
...
function Cell({
  ...
  return (
    <div className="flex flex-col items-center gap-1 min-w-[4.5rem]">
      <div
        className={`h-10 w-full rounded-md border border-white/10 ${scoreToHeatColor(score)} transition-colors`}
        title={`${label} (${scorePercent(score)})`}
      />
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3 text-[10px] text-white/45">
        ...
        <span className="inline-flex items-center gap-1">
          <span className="h-3 w-6 rounded bg-red-500" /> Bearish
        </span>
        <span className="inline-flex items-center gap-1">
          <span className="h-3 w-6 rounded bg-amber-500/50" /> Neutral
        </span>
        <span className="inline-flex items-center gap-1">
          <span className="h-3 w-6 rounded bg-emerald-500" /> Bullish
        </span>
      </div>
        ...
        <table className="w-full min-w-[640px] text-sm">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wide text-white/50">
              ...
              <th className="pb-3 px-2 font-semibold text-center">24</th>
            ...
            {rows.map((row) => (
              <tr key={row.source}>
                ...
                <td className="py-3 px-2 align-middle">
                  <Cell score={row.h24.score} label={row.h24.label} count={row.h24.count} />
                </td>
                <td className="py-3 px-2 align-middle">
                  <Cell score={row.week.score} label={row.week.label} count={row.week.count} />
                </td>
                <td className="py-3 px-2 align-middle">
                  <Cell score={row.month.score} label={row.month.label} count={row.month.count} />
                </td>
              </tr>
            ))}
          </tbody>
